        ...
        textRecognizer.process(visionImage) { (features, error) in
            ...
            for block in features.blocks{
                ...
                for line in block.lines {
                    let scanned = line.text
                    let test = String(scanned.filter { !" \n\t\r".contains($0) })
                    if Double(test) != nil{
                        print("Line \(sumline): \(test)")
                        self.listOfNumbers.append(test)
                        print("Added \(test) into List of Numbers.")
                        self.listOfLines.append(test)
                        print("Added \(test) into List of Lines.")
                    }
                    else{
                        print("Line \(sumline)L \(scanned)")
                        self.listOfLines.append(scanned)
                        print("line \(sumline): \(scanned)")
                    }
                    sumline += 1
